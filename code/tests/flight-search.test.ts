...
test('показывает пустой результат для одинаковых городов', async ({ page }) => {
  const requests = await openSearchPage(page, [{ body: flights }, { body: [] }])
  ...
  expect(requests[1].searchParams.get('origin')).toBe('MOW')
  expect(requests[1].searchParams.get('destination')).toBe('MOW')
})
...
test('реальный сервер возвращает пустой результат для одинаковых городов', async ({ page }) => {
  await page.goto('/')
